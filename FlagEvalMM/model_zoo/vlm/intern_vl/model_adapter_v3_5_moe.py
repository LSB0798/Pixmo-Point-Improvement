...
def split_model(model_name, model_path):
    ...
    print(f"Model: {model_name}, Available GPUs: {world_size}")
    
    # 根据 InternVL3.5-30B-A3B 的特点配置层数
    if "InternVL3_5" in model_name or "InternVL3.5" in model_name:
        # InternVL3.5-30B-A3B 特定配置
        num_layers = 48  # 根据实际模型配置调整
    else:
        config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
        num_layers = config.llm_config.num_hidden_layers
    
    # 第一个 GPU 用于 ViT，按 0.5 个 GPU 计算
