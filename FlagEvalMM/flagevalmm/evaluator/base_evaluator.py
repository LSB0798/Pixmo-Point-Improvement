...
from flagevalmm.evaluator.pre_process import process_multiple_choice, normalize_string, strip_special_tags, strip_special_tags_for_predictions
...
class BaseEvaluator:
    ...
    def evaluate_multiple_choice(self, gt: Dict, pred: Dict) -> bool:
        ...
        # 统一成大写，只保留字母，若有多个只取第一个
        gt["answer"] = re.findall(r"[A-Z]", gt["answer"].upper())[0]
        is_correct = bool(gt["answer"] == pred["answer"])
        return is_correct
    ...
    def _process_single_inference(
        ...
        predictions = strip_special_tags_for_predictions(predictions)
    ...
    def _process_multi_inference(
        ...
        expanded_predictions = strip_special_tags_for_predictions(expanded_predictions)
    ...
    def save(
        ...
        # for item in answers_copy:
        #     for q in item.get("question", []):
        #         if isinstance(q, dict):
        #             q.pop("image_url", None)
        answers_copy = sorted(answers_copy, key=lambda x: x.get("question_id", ""))
        json.dump(
            answers_copy,
