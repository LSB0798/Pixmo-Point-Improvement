    ...
    def run(self) -> None:
        for task_name in self.tasks:
            ...
            try:
                ...
                if str(e) == "AbortIter":
                    logger.info(f"[ABORT] AbortIter detected, stop evaluating ALL tasks for model {self.model_name}")
                    return  # return 或 break，取决于你希望整个进程结束还是只退出循环
                raise  # 其他 RuntimeError 继续抛
            self.task_manager.submit(task_name, self.model_name, meta_info["output_dir"])
