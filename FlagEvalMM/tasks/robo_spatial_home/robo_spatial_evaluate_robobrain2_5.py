...
def _parse_ab_choice(text: str):
    """
    Parse model output for binary multiple-choice: (A)/(B).
    Accepts: "(A)", "(B)", "A", "B", "Answer: (A)" etc.
    Returns: "a" / "b" / None
    """
    if text is None:
        return None
    ...
    # Prefer explicit parenthesis format
    m = re.search(r"\(\s*([ab])\s*\)", s)
    if m:
        return m.group(1)

    # Fallback: leading token "A" / "B"
    m = re.match(r"^\s*([ab])\b", s)
    if m:
        return m.group(1)

    # Fallback: "answer: a" / "final: b" etc.
    m = re.search(r"\b(answer|final|output)\s*[:\-]?\s*([ab])\b", s)
    if m:
        return m.group(2)
...
def evaluate_answer(ground_truth, generated_answer, width=None, height=None):
    ...
    If width and height are provided and the parsed point is in [0,1000] range,
    the point is scaled to pixel coordinates by (x/1000*width, y/1000*height).
    """
    def _maybe_scale(x, y):
        if width is not None and height is not None and 0 <= x <= 1000 and 0 <= y <= 1000:
    ...
    # Check if this is a binary yes/no question (now evaluated as (A)/(B))
    if gt_lower in ["yes", "no"]:
        is_binary = True
        expected = "a" if gt_lower == "yes" else "b"

        choice = _parse_ab_choice(generated_answer)
        is_parsable = choice is not None
        parsed_answer = f"({choice.upper()})" if choice is not None else None
...
def text2pts(text, width=640, height=480):
    ...
    if matches == []:
        ...
        matches = [(int(x), int(y)) for x, y in pattern.findall(text)]
    points = []
    for match in matches:
        ...
        if len(vector) == 2:
            ...
            #     x1 = int(x1 * width)
            #     y1 = int(y1 * height)
            x0, y0 = (x0 / 1000) * width, (y0 / 1000) * height
            x1, y1 = (x1 / 1000) * width, (y1 / 1000) * height
...
def evaluate_answer_v2(gt, normed_answer):
    ...
    if gt_lower in ["yes", "no"]:
        expected = "a" if gt_lower == "yes" else "b"
        choice = _parse_ab_choice(normed_answer)
        if choice is None:
            s = normed_answer.strip().lower()
            if s.startswith("yes"):
                choice = "a"
...
def get_result(annotations: Dict, predictions: List[Dict]) -> Dict:
    ...
    for pred in predictions:
        ...
        correct, is_binary, parsed_answer, is_parsable = evaluate_answer(
            gt["answer"], norm_pred, gt.get("image_width"), gt.get("image_height")
        )
