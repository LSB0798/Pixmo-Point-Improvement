...
python babysitting_multi.py \
  ...
  --extra-args "--limit-mm-per-prompt '{\"image\": 512, \"video\": 2}' \
                ...
                --data-parallel-size 1 --seed 3407 --max-model-len 100000 --mm-encoder-tp-mode data --trust-remote-code --gpu-memory-utilization 0.85 --allowed-local-media-path /code1 --interleave-mm-strings"  \
  --is-lora false \
  --is-mcore false \
