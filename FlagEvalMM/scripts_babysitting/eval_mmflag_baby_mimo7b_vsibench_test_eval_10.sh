...
python babysitting_multi.py \
  ...
  --tasks tasks/vsi_bench/vsi_bench_test_robix.py \
  --cuda 6,7 \
  ...
  --extra-args "--limit-mm-per-prompt '{\"image\": 512, \"video\": 2}' \
                ...
                --data-parallel-size 2 --seed 3407 --max-model-len 50000 --mm-encoder-tp-mode data --trust-remote-code --gpu-memory-utilization 0.4 --allowed-local-media-path /code1 --interleave-mm-strings"  \
  --is-lora false \
  --is-mcore false \
