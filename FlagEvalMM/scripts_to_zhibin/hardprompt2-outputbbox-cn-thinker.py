...
DEFAULT_SYSTEM_PROMPT = """你是一个专业的充电柜指示灯检测助手。你的任务是：在用户提供的充电柜图片中，仅检测位于“充电柜中间黑色背板区域”内的指示灯灯条，并输出其坐标框（bbox），例如：
```json
[
...
B) 数量与排列：目标最多只有两条，且位于对应仓口开口/电池进出口的正上方（两个指示灯间距约 30 厘米）。
C) 颜色状态：指示灯可能的状态包括：
- "green"：指示灯呈绿色；
- "yellow"：指示灯呈黄色或者橙色；
- "red"：指示灯呈红色；
...
E) 相对关系（可选锚点）：若图片中能看到对应仓口开口/电池进出口，则目标指示灯位于其正上方。
...
C) 若指示灯未发光（呈无光白色），也请输出其坐标。
D) 指示灯可能偏暖色（黄/橙），亮度很低，也要输出其坐标。
E) 以下目标无论多显眼都必须排除，不要当成指示灯：
    ...
    for stem, rec in sorted(gt_map.items()):
        ...
        if isinstance(pred, dict):
            ...
            if not pred_bboxes and isinstance(pred.get("raw_text"), str):
                print(f"[INFO] 尝试从 raw_text 解析 Pred bbox: stem={stem}")
...
def get_bboxes_from_any_obj(obj: Any) -> List[List[float]]:
    ...
    for k in ("target_bbox_2d", "bbox_2d", "bboxes", "bbox", "raw_text"):
        if k in obj:
    ...
    for k in ("pred", "output", "result", "prediction", "raw_text"):
        if k in obj:
...
# CUDA_VISIBLE_DEVICES=0 python light_sh_yjp/hardprompt2-outputbbox-cn-thinker.py all \
...
# --out_dir    light_results_yjp/260122-hardprompt2-outputbbox-cn-thinker \
# --iou_thr    0.5 \
# --batch_size 64

# CUDA_VISIBLE_DEVICES=1 python light_sh_yjp/hardprompt2-outputbbox-cn-qwen3vl.py vis \
# --pred_dir ./light_results_yjp/260122-hardprompt2-outputbbox-cn-qwen3vl/preds \
# --gt_set  ./light_data/labels/green_green_val.jsonl \
# ./light_data/labels/green_yellow_val.jsonl \
# ./light_data/labels/yellow_yellow_val.jsonl \
# ./light_data/labels/yellow_green_val.jsonl \
...
# ./light_data/labels1225/yellow_yellow_val.jsonl \
# --out_dir    light_results_yjp/260122-hardprompt2-outputbbox-cn-qwen3vl \
# --iou_thr    0.5
