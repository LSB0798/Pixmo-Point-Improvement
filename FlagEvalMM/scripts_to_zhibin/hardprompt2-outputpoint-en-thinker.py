...
DEFAULT_SYSTEM_PROMPT = """You are a professional charging-cabinet indicator light detection assistant. Your task is: in the charging-cabinet image provided by the user, identify ONLY the indicator light bars located within the “central black backboard area” of the charging cabinet, and output their coordinates (points), e.g.:
[(x1, y1), (x2, y2), ..., (xn, yn)]

[Strict Definition of the Target Indicator Lights]
A) Location: The target indicator lights must be located within the central black backboard area of the charging cabinet.
B) Count and arrangement: There are at most two target light bars, and they are positioned directly above the corresponding bay opening / battery inlet-outlet (the distance between the two indicator lights is about 30 cm).
C) Color states: Possible indicator states include:
...
- "off": the indicator light bar is clearly visible, but there is no obvious glow/highlight; it appears white/gray;
- "unknown": the indicator light is occluded or too blurry to be recognized;
D) Size: The target light bar is about 30 cm long and about 4 cm wide; any clearly longer light strips / ambient lights / decorative lights must be ignored.
E) Relative relation (optional anchor): If the corresponding bay opening / battery inlet-outlet is visible in the image, the target indicator light is directly above it.

[Notes]
A) If there are more than two candidates:
- Prefer those that best match “about 30 cm long and about 4 cm wide”, with the two bars having the most consistent sizes, and both located directly above the bay openings / battery inlet-outlets;
- Ignore light strips that are clearly longer, thinner, intermittent, or deviating from the core area of the backboard.
B) If there are fewer than two candidates, the indicator light(s) may be occluded. For an occluded indicator light, its corresponding entry should be omitted; return only visible indicator light results.
C) Even if an indicator light is not lit (appearing as an unlit white bar), still output its coordinates.
D) The indicator light may be warm-toned (yellow/orange) and very dim; still output its coordinates.
E) The following items must be excluded as targets no matter how salient they are, and must NOT be treated as indicator lights:
...
def _infer_batch_from_gt_records_vllm(
    ...
    for ridx, (stem, img_path, raw) in enumerate(records):
        ...
        query_content.append({
            "type": "text",
            "text": DEFAULT_SYSTEM_PROMPT + DEFAULT_USER_TEXT,
        })
...
# CUDA_VISIBLE_DEVICES=1 python light_sh_yjp/hardprompt2-outputpoint-en-thinker.py all \
...
# --out_dir    light_results_yjp/260122-hardprompt2-outputpoint-en-thinker \
# --iou_thr    0.5 \
# --batch_size 32
