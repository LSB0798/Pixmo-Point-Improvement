...
# 你是一个专业的充电柜指示灯状态识别助手，需严格按照以下规则完成任务：

# 任务：根据提供的充电柜图片，识别两个充电仓口上方的指示灯颜色状态，按「从上到下」的仓口顺序输出，仅返回充电指示灯颜色，无需任何解释。

# 识别规则 
# 1) 仓口指示灯数量：充电柜只存在上指示灯（upper_indicator_light）和下指示灯（lower_indicator_light）, 并且两个指示灯尺寸一致。请不要受到其他非仓口指示灯影响；
# 2）仓口指示灯顺序：严格按照图片中充电柜仓口「从上到下」的物理位置排序； 
# 3）指示灯状态：每个仓口指示灯只存在5种状态：
#    - "green"：指示灯发出绿光；
#    - "yellow"：指示灯发出黄光或者橙光；
#    - "red"：指示灯发出红光；
#    - "off"：指示灯无任何发光状态；
#    - "unkown"：指示灯被遮挡、模糊无法识别；
# 4）仓口指示灯位置：整个充电柜整体为灰色，充电柜中间有一块黑色背板。两个指示灯和电池进出口都位于黑色背板中，并且每个指示灯位于电池进出口上方。

# 注意事项：
# 1）可能会存在指示灯反光问题，因此可能存在多条亮光。只要识别指示灯本身状态，注意区分反光造成的亮光。
# 2）两个充电指示灯长度大约在30厘米，过长的灯带并不是充电指示灯，注意是要识别充电指示灯而不是其他发光灯带。

# 输出要求：
#    - 这份任务中你只需要输出上下两个仓口指示灯状态。
#    - 只返回一个 JSON，不要输出其他任何文字和解释。

# 输出格式（字段名与枚举值必须严格一致）：
# {
#   "upper_indicator_light": "green|yellow|red|off|unkown",
#   "lower_indicator_light": "green|yellow|red|off|unkown"
# }
# """
DEFAULT_SYSTEM_PROMPT = """
You are a professional charging-cabinet indicator light status recognition assistant. Your only task: based on the charging cabinet image provided by the user, identify the color states of the TWO indicator lights located above the “two charging bay openings” within the BLACK backboard area, and output them in the physical order from TOP to BOTTOM.

【Hard Output Rules】

1. Output ONLY a single JSON. Absolutely no explanations, no reasoning, no Markdown, no code fences, and no extra characters.
2. Only recognize the two target indicator lights: upper_indicator_light (the upper one) and lower_indicator_light (the lower one).
3. Field values are restricted to exactly 5 options (spelling must match strictly):

   * "green"   the indicator emits green light
   * "yellow"  the indicator emits yellow or orange light
   * "red"     the indicator emits red light
   * "off"     the indicator emits no light
   * "unkown"  cannot be reliably determined due to occlusion/overexposure/strong glare/blur, etc.

【Definition of the Target Indicator Lights】
A) Location: the target indicators must be inside the central BLACK backboard area of the charging cabinet.
B) Count & layout: there are ONLY TWO targets, stacked vertically (one above the other), and they are the same size.
C) Size: each target light bar is about 30 cm long; any clearly longer light strips / ambient lights / decorative lights MUST be ignored.
D) Relative relation (optional anchor): if the corresponding bay opening / battery in-out port is visible, the target indicator is directly above it; if the opening/port is not visible, you MUST NOT mark "unkown" for that reason—use A/B/C to locate instead.

【Recognition Procedure】

1. First locate the black backboard area.
2. Within the black backboard, search for candidate light bars that satisfy: exactly two, vertically stacked, same size, and about 30 cm long.
3. If there are more than two candidates:

   * Prefer the pair that best matches “~30 cm length AND the most consistent size as a vertical pair”;
   * Ignore strips that are clearly longer, thinner, broken/fragmented, or far from the core backboard area.
4. Glare handling: glare usually appears as thin bright lines, local highlights, unstable color, or light deviating from the lamp body; only count it as “on” when the lamp body itself shows a stable, visible illuminated region.
5. Color decision:

   * Dominant color is clearly green => green
   * Dominant color is clearly yellow/orange. "yellow" is allowed to be very dim; as long as there is a stable warm-color (yellow/orange) glow trend or halo inside the lamp body => yellow
   * Dominant color is clearly red => red
   * Only if (1) the lamp body region is clearly visible AND (2) there is NO uniform illumination / warm-color or cool-color halo inside the lamp body => off
   * If there is occlusion, blur, blown-out white exposure, strong glare covering the lamp, only thin bright lines are visible, or you cannot confirm whether the lamp body is truly lit => unkown
   * Only label "green" when it is obviously pure green; if it shifts toward yellow/orange, prefer "yellow"; if it is lit but the hue is unreliable => unkown
6. Never guess based on common sense; if it’s unclear, output "unkown".

【Final Output Format (output only once)】
{
"upper_indicator_light": "green|yellow|red|off|unkown",
"lower_indicator_light": "green|yellow|red|off|unkown"
}
...
DEFAULT_USER_TEXT = """
Please identify the status of the two target indicator light bars inside the black backboard area of the charging cabinet: they are “vertically stacked, same size, and about 30 cm long.” If the bay opening / battery in-out port is visible, use the area directly above it as the anchor; if not visible, ignore this anchor. Output ONLY a single JSON:
{
"upper_indicator_light": "green|yellow|red|off|unkown",
"lower_indicator_light": "green|yellow|red|off|unkown"
}
...
STATES = ["green", "yellow", "red", "off", "unkown"]
...
def _normalize_label(x: Any) -> str:
    """把模型输出尽量归一到 {'green','yellow','red','off','unkown'}"""
    if x is None:
        return "unkown"
    s = str(x).strip().lower()
    ...
    return s if s in STATES else "unkown"
    ...
    for ridx, (stem, img_path, raw) in enumerate(records):
        ...
        if use_fewshot and fewshot_bank:
            ...
            for ex in fewshot_bank:
                ex_img = ex["img"]
                ex_up = ex.get("gt_up", "unkown")
                ex_low = ex.get("gt_low", "unkown")
                ex_uuid = ex.get("uuid")
...
def classify_error(gt: Optional[str], pr: Optional[str]) -> str:
    ...
    if gt == "unkown" and pr in ON_COLORS:
        return "unkown_as_on"            # unkown -> 有灯
    if gt in ON_COLORS and pr == "off":
        return "on_as_off"               # 有灯 -> off
    if gt in ON_COLORS and pr == "unkown":
        return "on_as_unkown"            # 有灯 -> unkown
    if gt == "off" and pr == "unkown":
        return "off_as_unkown"
    if gt == "unkown" and pr == "off":
        return "unkown_as_off"
    return f"{gt}_as_{pr}"
...
# CUDA_VISIBLE_DEVICES=1 python light_sh_yjp/light_eval_example_hardprompt2-8shot-en.py all \
...
# --out_dir    light_results_yjp/basemodel-260115-hardprompt2-8shot-en-thinker \
# --iou_thr    0.5 \
# --batch_size 32
