...
DEFAULT_SYSTEM_PROMPT = """你是一个专业的充电柜指示灯检测助手。你的任务是：在用户提供的充电柜图片中，仅识别位于“充电柜中间黑色背板区域”内的指示灯灯条，并输出其坐标框（bbox），最多输出 2 条
```json
[
...
B) 数量与排列：目标最多只有两条，且位于对应仓口开口/电池进出口的正上方（指示灯之间有一定距离）。
C) 颜色状态：指示灯可能的状态包括：
- "green"：指示灯发出绿光；
- "yellow"：指示灯发出黄光或者橙光；
- "red"：指示灯发出红光；
- "off"：指示灯实体清晰，但没有任何明显发光/高亮溢出；
- "unkown"：指示灯被遮挡、模糊无法识别；
D) 尺寸：目标灯条长度约 30 厘米，宽度约 4 厘米；明显更长的灯带/氛围灯/装饰灯一律忽略。
E) 相对关系（可选锚点）：若图片中能看到对应仓口开口/电池进出口，则目标指示灯位于其正上方，在多候选时，优先选择“最靠近对应仓口上方”的那一对。

【注意事项】
A) 若候选多于两条：
- 优先选择最符合“长度约 30cm，宽度约 4 厘米且两条尺寸最一致、上下成对，处于仓口开口/电池进出口正上方”的那一对；
- 忽略明显更长、更细、断续、偏离背板核心区域的光条。
B) 若候选少于两条，指示灯可能被遮挡，被遮挡的指示灯对应列表为空，只返回可见的指示灯结果。
C) 若指示灯未发光（呈无光白色），也请框出其位置。
D) 指示灯可能偏暖色（黄/橙），亮度很低，也需要框出来。
E) 以下目标无论多显眼都必须排除，不要框成指示灯：
- 二维码/条码/文字标签（通常是方形或带网格/文字纹理）
- 红色按钮/急停/圆形旋钮（圆形或小面积块）
- 充电口/插孔/USB口/圆孔矩形孔（孔洞形态，不是长条扩散板）
...
def visualize_annotations_from_gtset(
    ...
    for stem, rec in sorted(gt_map.items()):
        ...
        # ---------------------------
        # 1) 抽取 GT bbox（如果你的 GT 也有 target_bbox_2d / bbox_2d）
        # ---------------------------
        gt_bboxes = get_bboxes_from_any_obj(gt)

        # ---------------------------
        # 2) 抽取 Pred bbox：支持 pred.json 里直接有字段，也支持从 raw_text 里解析
        # ---------------------------
        pred_bboxes: List[List[float]] = []

        if isinstance(pred, dict):
            # (a) 直接从 pred dict 里抽
            pred_bboxes = get_bboxes_from_any_obj(pred)

            # (b) 如果没抽到，尝试从 raw_text 里抽 JSON 再解析 bbox
            if not pred_bboxes and isinstance(pred.get("raw_text"), str):
                js = extract_json_any(pred["raw_text"])
                if js:
                    try:
                        obj = json.loads(js)
                        pred_bboxes = get_bboxes_from_any_obj(obj)
                    except Exception:
                        pass
        ...
        for i, bb in enumerate(gt_bboxes):
            try:
                x1, y1, x2, y2 = bbox_to_pixel_xyxy(bb, w, h)
                img = draw_box_cv2(img, x1, y1, x2, y2, gt_color, label=f"GT[{i}]")
            except Exception as e:
                print(f"[WARN] GT bbox 绘制失败: stem={stem} bb={bb} err={e}")

        for i, bb in enumerate(pred_bboxes):
            try:
                x1, y1, x2, y2 = bbox_to_pixel_xyxy(bb, w, h)
                img = draw_box_cv2(img, x1, y1, x2, y2, pred_color, label=f"Pred[{i}]")
            except Exception as e:
                print(f"[WARN] Pred bbox 绘制失败: stem={stem} bb={bb} err={e}")
        ...
        # ---------------------------
        all_correct = bool(pred_bboxes)  # 先粗暴：有框就算 true（你可以改成 IoU/匹配逻辑）
...
def _as_bbox_list(v: Any) -> List[List[float]]:
    """
    把各种可能形式的 bbox 字段统一成 List[[x1,y1,x2,y2], ...]
    支持：
      - [x1,y1,x2,y2]
      - [[...],[...]]
      - [{"bbox_2d":[...]}, ...]
    ...
    if isinstance(v, list):
        ...
        for item in v:
            if isinstance(item, list) and len(item) == 4 and all(isinstance(x, (int, float)) for x in item):
                ...
                bb = item.get("bbox_2d") or item.get("target_bbox_2d")
...
def get_bboxes_from_any_obj(obj: Any) -> List[List[float]]:
    """
    从任意 obj 里尽量抽取 bbox 列表。
    优先找：target_bbox_2d / bbox_2d / bboxes / bbox
    也支持 obj 本身就是 list（比如模型直接输出 [ {"bbox_2d":...}, ... ]）
    """
    if obj is None:
        return []

    # obj 本身就是 list（比如直接输出 array）
    if isinstance(obj, list):
        # 可能是 [[...],[...]] 或 [{"bbox_2d":...}, ...]
        bbs = _as_bbox_list(obj)
        if bbs:
            return bbs
        return []
    ...
    for k in ("target_bbox_2d", "bbox_2d", "bboxes", "bbox"):
        if k in obj:
            bbs = _as_bbox_list(obj.get(k))
            if bbs:
                return bbs
    ...
    for k in ("pred", "output", "result", "prediction"):
        if k in obj:
            bbs = get_bboxes_from_any_obj(obj.get(k))
            if bbs:
                return bbs
...
def bbox_to_pixel_xyxy(bbox: List[float], img_w: int, img_h: int) -> Tuple[int, int, int, int]:
    """
    把 bbox 转成像素 xyxy。
    自动判断 bbox 坐标系：
      - max<=1.5  -> 认为是 0~1 归一化
      - max<=1005 -> 认为是 0~1000 坐标系
      - 否则      -> 认为已是像素坐标
    并做 clamp + 保证 x1<=x2,y1<=y2
    """
...
# CUDA_VISIBLE_DEVICES=7 python light_sh_yjp/light_eval_example_hardprompt2-outputbbox-only-userprompt-cn.py all \
...
# --out_dir    light_results_yjp/basemodel-260122-hardprompt2-outputbbox-only-userprompt-cn-qwen3vl \
# --iou_thr    0.5 \
# --batch_size 32
