...
DEFAULT_USER_TEXT_1 = """You are a professional assistant for recognizing the status of indicator lights on a charging cabinet. Your only task is: based on the charging cabinet image provided by the user, identify the color/status of the two indicator lights within the black backboard area.

【Hard Rules for Output】
1) Output only ONE JSON object.
2) Only recognize the two target indicator lights: upper_indicator_light (upper indicator light) and lower_indicator_light (lower indicator light).

【Definition of the Target Indicator Lights】
1) Location: The target indicator lights must be located within the black backboard area in the middle of the charging cabinet.
2) Count and arrangement: There are at most two target lights, each directly above the corresponding bay opening / battery inlet-outlet (the distance between the two lights is about 30 cm).
3) Color/status: Possible states include:
- "green": the indicator light appears green;
- "yellow": the indicator light appears yellow or orange;
- "red": the indicator light appears red;
- "off": the light strip body is clearly visible, but there is no obvious emission/highlight; it appears whitish-gray;
- "unknown": the indicator light is occluded or too blurry to recognize;
4) Size: The target light strip is about 30 cm long and about 4 cm wide; any clearly longer light strips / ambient lights / decorative lights must be ignored.
5) Relative relationship (optional anchor): If the corresponding bay opening / battery inlet-outlet is visible in the image, the target indicator light is directly above it.

【Notes】
1) If there are more than two candidates:
- Prefer the two light strips that best match “about 30 cm long and about 4 cm wide,” are most consistent in size with each other, and are both directly above the bay openings / battery inlet-outlets;
- Ignore light strips that are clearly longer, thinner, intermittent, or that deviate from the core area of the backboard.
2) If there are fewer than two candidates, the missing indicator light(s) may be occluded. For occluded indicator lights, return an empty list for that corresponding item, and only return recognition results for the visible indicator lights.
3) If the indicator light is not lit (appears as unlit white), the status is off.
4) The indicator light may be warm-toned (yellow/orange) and relatively dim; in that case the status is yellow.
5) The following items must be excluded as targets no matter how salient they are, and must NOT be treated as indicator lights:
...
6) Do NOT guess based on common sense; if it’s unclear, classify it as unknown.
            ...
            for ex in fewshot_bank:
                ...
                messages.append({
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": DEFAULT_USER_TEXT_1}
                    ],
                })
    ...
    for meta, text in zip(metas, outputs_text):
        ...
        if not parse_ok:
            try:
                ...
                pass
        # 兼容选择题输出：A, D / "option_upper=A option_lower=D" 等
        if not parse_ok:
            opts = re.findall(r"(?i)\b([A-E])\b", raw_text)
            if len(opts) >= 2:
                pred_up = _normalize_label(opts[0])
                pred_low = _normalize_label(opts[1])
                parse_ok = True
...
# CUDA_VISIBLE_DEVICES=1 python light_sh_yjp/hardprompt-end2end-en-thinker.py all \
...
# ./light_data/labels1225/yellow_unknown_val.jsonl \
# ./light_data/labels1225/yellow_yellow_val.jsonl \
# --out_dir    light_results_yjp/260128-hardprompt-end2end-en-thinker \
# --iou_thr    0.5 \
# --batch_size 32 \
