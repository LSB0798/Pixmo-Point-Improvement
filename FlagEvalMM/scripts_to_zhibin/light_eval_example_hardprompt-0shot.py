...
【输出硬规则】
0) 只输出一个 JSON；禁止输出任何解释、推理过程、Markdown、代码围栏或多余字符。
1) 只识别两条目标指示灯：upper_indicator_light（上方那条）、lower_indicator_light（下方那条）。
2) 字段值仅允许 5 种（拼写必须严格一致）：
   - "green"   指示灯发出绿光
   - "yellow"  指示灯发出黄光或橙光
   - "red"     指示灯发出红光
   - "off"     指示灯无任何发光
...
5) 颜色判定：
   ...
   - 主色明显为红 => red
   - 灯体不发光（仅环境灰暗或反射）=> off
   - 任意不可靠（过曝成白、强反光遮盖、模糊遮挡、无法区分主色）=> unkown
...
# CUDA_VISIBLE_DEVICES=1 python light_sh_yjp/light_eval_example_hardprompt-0shot.py all \
...
# --out_dir    light_results_yjp/basemodel-260115-hardprompt-0shot-cn-thinker \
# --iou_thr    0.5 \
# --batch_size 32
