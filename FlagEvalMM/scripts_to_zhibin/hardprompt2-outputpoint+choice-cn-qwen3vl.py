...
你是一个专业的充电柜指示灯【定位 + 状态识别】助手。你必须在同一张图片上完成两个子任务，并在内部推理中严格按顺序执行：
(1) 先定位指示灯灯条并确定其 point；
(2) 再基于 point 处的灯条识别其颜色状态。
你的最终回复必须【只输出一个 JSON】，不得输出任何解释、文字、Markdown、代码块围栏。

【目标指示灯的严格定义】
A) 位置：目标指示灯必须位于“充电柜中间黑色背板区域”内。
B) 数量与排列：目标最多只有两条，且位于对应仓口开口/电池进出口的正上方（两条之间间距约 30 厘米）。
C) 尺寸：目标灯条长度约 30 厘米，宽度约 4 厘米；明显更长的灯带/氛围灯/装饰灯一律忽略。
D) 颜色状态（选择题输出）：
- A.green：指示灯呈绿色
- B.yellow：指示灯呈黄色或橙色（偏暖也算）
- C.red：指示灯呈红色
- D.off：灯条实体清晰，但无明显发光/高亮（白灰色/不亮）
- E.unknown：被遮挡、模糊、过曝、无法可靠判断
E) 相对关系（可选锚点）：若能看到仓口开口/电池进出口，目标指示灯在其正上方。

【必须排除的干扰目标（无论多显眼都不要当成指示灯）】
- 二维码/条码/文字标签（方形、网格纹理、文字纹理）
- 红色按钮/急停/圆形旋钮（圆形或小面积块）
- 充电口/插孔/USB口/圆孔矩形孔（孔洞形态）

【任务执行流程（必须遵守，内部推理顺序不可颠倒）】
Step 1) 仅在“中间黑色背板区域”内寻找候选灯条。
Step 2) 若候选多于两条：
  - 优先选择最符合“约30cm×4cm”的两条；
  - 两条尺寸最一致、且更接近仓口上方、且位于背板核心区域者优先；
  - 忽略明显更长、更细、断续、偏离背板核心区域的光条。
  若候选少于两条，指示灯可能被遮挡，被遮挡的指示灯对应列表为空，只返回可见的指示灯结果。
Step 3) 为每条选中的指示灯输出 point：
  - point_2d 为灯条【中心点】像素坐标 [x, y]（x 向右增大，y 向下增大）。
Step 4) 基于 Step 3 选中的灯条区域再识别颜色状态：
  - 严禁基于常识猜测；看不清就 E.unknown；
  - 低亮度偏暖色也应判为 B.yellow；
  - 灯条实体清晰但不发光判为 D.off。
Step 5) upper/lower 规则：
  - 若检测到两条：y 更小者为 upper_indicator_light；y 更大者为 lower_indicator_light。
  - 若只检测到一条：按其在画面中的相对位置命名（更靠上则 upper，否则 lower），另一条输出 unknown 且 point 为 null。
  - 若一条都检测不到：两条均输出 unknown，point 均为 null。

========================
【最终输出格式（只输出一个 JSON）】
{
  "points": {
    "upper_indicator_light": [x, y] | null,
    "lower_indicator_light": [x, y] | null
  },
  "states": {
    "upper_indicator_light": "A|B|C|D|E",
    "lower_indicator_light": "A|B|C|D|E"
  }
}

注意：
1) points 里只能是 [x,y] 或 null。
2) states 里只能是 A/B/C/D/E 之一。
3) 不得输出除 JSON 之外的任何内容。
"""
...
def _normalize_label(x: Any) -> str:
    ...
    # 常见同义/中英文映射
    if s in {"unknown", "unknown", "?", "n/a", "na", "none", "null", "uncertain", "unclear", "不确定", "未知", "看不清", "e"}:
        return "unknown"
    if s in {"green", "g", "绿", "绿色", "a"}:
        return "green"
    if s in {"yellow", "amber", "y", "黄", "黄色", "b"}:
        return "yellow"
    if s in {"red", "r", "红", "红色", "c"}:
        return "red"
    if s in {"off", "dark", "black", "灭", "熄灭", "不亮", "无灯", "d"}:
        return "off"
...
def _infer_batch_from_gt_records_vllm(
    ...
    for meta, text in zip(metas, outputs_text):
        ...
        if isinstance(obj, dict):
            ...
            parse_ok = (up is not None) and (low is not None)
...
# CUDA_VISIBLE_DEVICES=0 python light_sh_yjp/hardprompt2-outputchoice-cn-qwen3vl.py all \
...
# --out_dir    light_results_yjp/260122-hardprompt2-outputchoice-cn-qwen3vl \
# --iou_thr    0.5 \
# --batch_size 32
...
# --gt_set ./light_data/labels_20/green_green_val.jsonl \
# ./light_data/labels_20/green_yellow_val.jsonl \
# ./light_data/labels_20/yellow_yellow_val.jsonl \
# ./light_data/labels_20/yellow_green_val.jsonl \
# ./light_data/labels1225_20/off_off_val.jsonl \
# ./light_data/labels1225_20/off_unknown_val.jsonl \
# ./light_data/labels1225_20/red_red_val.jsonl \
# ./light_data/labels1225_20/unknown_off_val.jsonl \
# ./light_data/labels1225_20/unknown_red_val.jsonl \
# ./light_data/labels1225_20/unknown_unknown_val.jsonl \
# ./light_data/labels1225_20/unknown_yellow_val.jsonl \
# ./light_data/labels1225_20/yellow_unknown_val.jsonl \
# ./light_data/labels1225_20/yellow_yellow_val.jsonl \
