...
# """
DEFAULT_SYSTEM_PROMPT = """
你是一个专业的充电柜指示灯状态识别助手。你的唯一任务：根据用户提供的充电柜图片，识别黑色背板区域内“两个充电仓口”上方的两条指示灯的颜色状态，并按“从上到下”的物理顺序输出。

【输出硬规则】
1) 只输出一个 JSON；禁止输出任何解释、推理过程、Markdown、代码围栏或多余字符。
2) 只识别两条目标指示灯：upper_indicator_light（上方那条）、lower_indicator_light（下方那条）。
3) 字段值仅允许 5 种（拼写必须严格一致）：
   - "green"   指示灯发出绿光
   - "yellow"  指示灯发出黄光或橙光
   - "red"     指示灯发出红光
   - "off"     指示灯无任何发光
   - "unkown" 被遮挡/过曝/强反光/模糊等导致无法可靠判断

【目标指示灯的定义】
A) 位置：目标指示灯必须位于充电柜中间的黑色背板区域内。
B) 数量与排列：目标只有两条，且上下排列，尺寸一致。
C) 尺寸：两条目标灯条长度约 30 厘米；明显更长的灯带/氛围灯/装饰灯一律忽略。
D) 相对关系（可选锚点）：若图片中能看到对应仓口开口/电池进出口，则目标指示灯位于其正上方；若看不到开口/进出口，不得因此判 unkown，改用 A/B/C 的定义定位。

【识别流程】
1) 先定位黑色背板区域。
2) 在黑色背板内寻找“满足：两条、上下排列、尺寸一致、长度约 30cm”的灯条作为候选目标。
3) 若候选多于两条：
   - 优先选择最符合“长度约 30cm 且两条尺寸最一致、上下成对”的那一对；
   - 忽略明显更长、更细、断续、偏离背板核心区域的光条。
4) 反光处理：反光通常表现为细亮线、局部高光、颜色不稳定或偏离灯体；只有当灯体本身呈现稳定、可见的发光区域时才算点亮。
5) 颜色判定：
   - 主色明显为绿 => green
   - 主色明显为黄/橙，yellow 允许很暗；只要灯体内部出现稳定的暖色（黄/橙）发光趋势或光晕 => yellow
   - 主色明显为红 => red
   - 只有当“指示灯灯体区域清晰可见”且“灯体内部没有任何均匀发光/暖色或冷色光晕”时 => off
   - 若存在遮挡、模糊、过曝变白、强反光覆盖、只能看到细亮线或无法确认灯体是否真的点亮 => unkown
   - 只有“明显纯绿”才判 green；只要偏黄/偏橙就优先判 yellow；发光但色相不可靠则判 unkown
6) 严禁基于常识猜测；看不清就输出 unkown。
...
DEFAULT_USER_TEXT = """
请识别充电柜黑色背板区域内，两条“上下排列、尺寸一致、长度约30cm”的目标指示灯状态；若能看到仓口开口/进出口则以其正上方为准，若看不到则忽略该锚点。只输出一个 JSON：
{
  "upper_indicator_light": "green|yellow|red|off|unkown",
  "lower_indicator_light": "green|yellow|red|off|unkown"
}
    ...
    for ridx, (stem, img_path, raw) in enumerate(records):
        ...
        if use_fewshot and fewshot_bank:
            ...
            for ex in fewshot_bank:
                ...
                ex_uuid = ex.get("uuid")
                user_tmp_prompt = f"这张图里有上下两个红框，分别对应了上指示灯和下指示灯。上指示灯状态为{ex_up}， 下指示灯状态为{ex_low}。\n"
                # if ex_up == "unkown":
                #     user_tmp_prompt = f"这张图里只有一个红框，对应了下指示灯。上指示灯被遮挡住了，没有用红框标出来。上指示灯状态为{ex_up}， 下指示灯状态为{ex_low}。\n"
                # if ex_low == "unkown" and ex_up != "unkown":
                #     user_tmp_prompt = f"这张图里只有一个红框，对应了上指示灯。下指示灯被遮挡住了，没有用红框标出来。上指示灯状态为{ex_up}， 下指示灯状态为{ex_low}。\n"
                if ex_up == "unkown":
                    user_tmp_prompt = f"这张图里只有一个红框，对应了上指示灯。下指示灯被遮挡住了，没有用红框标出来。上指示灯状态为{ex_up}， 下指示灯状态为{ex_low}。\n"
                if ex_low == "unkown" and ex_up != "unkown":
                    user_tmp_prompt = f"这张图里只有一个红框，对应了下指示灯。上指示灯被遮挡住了，没有用红框标出来。上指示灯状态为{ex_up}， 下指示灯状态为{ex_low}。\n"
                messages.append({
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": user_tmp_prompt}
                    ],
                })
...
# CUDA_VISIBLE_DEVICES=7 python light_sh_yjp/light_eval_example_hardprompt2-8shot-bbox.py all \
...
# --out_dir    light_results_yjp/basemodel-260119-hardprompt2-8shot-cn-qwen3vl-bbox \
# --iou_thr    0.5 \
# --batch_size 32
