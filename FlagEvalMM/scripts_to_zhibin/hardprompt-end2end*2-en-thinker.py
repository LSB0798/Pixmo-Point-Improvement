...
PROMPT_UP = """Please state the colors of the upper indicator lights in the black backplate area inside the charging cabinet shown in the image. Choose from the following options:
A. Green
B. Yellow
C. Red
...
PROMPT_LOW = """Please state the colors of the lower indicator lights in the black backplate area inside the charging cabinet shown in the image. Choose from the following options:
A. Green
B. Yellow
C. Red
D. off (indicator light is off)
...
B) If there are fewer than two candidates, the indicator light(s) may be occluded. For an occluded indicator light, return E. unknown.
...
PROMPT_BOTH3= """"
A) The target indicator lights must be located within the black backboard area in the middle of the charging cabinet.
B) Prefer those that best match “about 30 cm long and about 4 cm wide”, and located directly above the bay openings / battery inlet-outlets;
- Ignore light strips that are clearly longer, thinner, intermittent, or deviating from the core area of the backboard.
C) For an occluded indicator light, return E. unknown.
D) The following items must be excluded as targets no matter how salient they are, and must NOT be treated as indicator lights:
- QR codes / barcodes / text labels (usually square or with grid/text textures)
- Red buttons / emergency stops / round knobs (circular or small-area blocks)
- Charging ports / jacks / USB ports / round holes / rectangular holes (hole-like shapes)
E) Do not recognize reflections as indicator lights. Reflections usually appear as localized glare/flare or are offset from the light body; only count it as lit when the light body itself shows a stable, visible luminous area.
"""
...
def _infer_batch_from_gt_records_vllm(
    ...
    for ridx, (stem, img_path, raw) in enumerate(records):
        ...
        for which, user_text in (("up", PROMPT_UP), ("low", PROMPT_LOW)):
            messages = []  # 确保每次推理的 messages 是干净的

            # query：主图 + 指令
            query_content = [{"type": "image", "image": query_img, "uuid": f"{stem}-main"}]
            query_content.append({"type": "text", "text": user_text + PROMPT_BOTH3 + "\nAnswer with the option's letter from the given choices directly:"})
    ...
    for meta, text in zip(metas, outputs_text):
        ...
        if not parse_ok:
            ...
            if len(opts) >= 1:
                one = _normalize_label(opts[0])
                if meta.get("which") == "up":
                    pred_up = one
                elif meta.get("which") == "low":
                    pred_low = one
                else:
                    pred_up = one
                parse_ok = True
    ...
    # === 按 ridx 聚合：两次推理合成一条，并回填 final_results ===
    ...
    for r in partials:
        ...
        if r.get("which") == "up":
            agg["pred_up"] = r.get("pred_up", "unknown")
            agg["raw_text_up"] = r.get("raw_text", "")
            agg["parse_ok_up"] = bool(r.get("parse_ok"))
        elif r.get("which") == "low":
            agg["pred_low"] = r.get("pred_low", "unknown")
            agg["raw_text_low"] = r.get("raw_text", "")
            agg["parse_ok_low"] = bool(r.get("parse_ok"))

    # 回填到 final_results（保持 records 顺序 & 保留 failed_to_load_image）
    for ridx, agg in by_ridx.items():
        agg["parse_ok"] = agg["parse_ok_up"] and agg["parse_ok_low"]
...
# CUDA_VISIBLE_DEVICES=0 python light_sh_yjp/hardprompt-end2end*2-en-thinker.py all \
...
# ./light_data/labels1225/yellow_unknown_val.jsonl \
# ./light_data/labels1225/yellow_yellow_val.jsonl \
# --out_dir    light_results_yjp/260128-hardprompt3-end2end*2-en-thinker \
# --iou_thr    0.5 \
# --batch_size 32 \
