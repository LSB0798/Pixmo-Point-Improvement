...
# === Step 3: 动态负载均衡（队列 + 加锁轮询client）===
from concurrent.futures import ThreadPoolExecutor, as_completed
from queue import Queue
import threading
# 【新增】线程安全的Client轮询器：保证8线程并发时client均匀分配
class ClientRoundRobin:
    ...
    def get_next(self):
        with self.lock:  # 保证原子性，避免竞态
            client = self.clients[self.counter % len(self.clients)]
            self.counter += 1
            return client


class Pixmo_Point_Reward(ORM):
    """
    1, 8路API并行，问题在于样本难度不均衡，会有个别样本需要API进行长时间调用
    2, 导致结果就是7个API停止，1个API长时间跑
    """
    def __init__(self):
        pass
    ...
    def calculate_spatial_reward(self, image_path_or_b64, query, client=None):
        pass
    
    def get_pred_image(self, image_source_path, image_target_path, x, y, gap = 100):
        pass
    
    def get_spatial_reward(self, image_source_path, image_target_path, Query, content, sol, del_position, client=None):
        pass
    
    def _parse_points(self, s: str) -> Optional[np.ndarray]:
        pass
    
    def crop_and_encode_memory(self, image_source_path, x, y, gap):
        pass
...
orms = {
    'toolbench': ReactORM,
    'math': MathORM,
    'pixmo_point_reward': Pixmo_Point_Reward,
